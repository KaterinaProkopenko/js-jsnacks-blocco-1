...
for (let i = 0; i < 10; i++) {
    const nameRandom = Math.floor(Math.random() * nomi.length);
    const surnameRandom = Math.floor(Math.random() * cognomi.length);

    const nameRandomCorrect = nomi[nameRandom][0].toUpperCase() + nomi[nameRandom].substring(1);
    const surnameRandomCorrect = cognomi[surnameRandom][0].toUpperCase() + cognomi[surnameRandom].substring(1);

    const nameSurname = nameRandomCorrect + ' ' + surnameRandomCorrect;
    guests.push(nameSurname);
}
